...
Last weekend I went to [my first hackathon](https://codebar.io/events/gender-pay-gap-hack). It was organised by the [AI club for gender minorities](https://www.meetup.com/ai-club/), [codebar](https://codebar.io/) and [ellpha](https://www.ellpha.com/). We used data on the gender pay gap [available here](https://gender-pay-gap.service.gov.uk/). I had a great time so I wanted to blog about it. This is the first part. 

## The deep dive team
There were different tables with different themes and I picked deep dive. This was unrelated to deep learning but it was for anyone who wanted to focus on one specific thing. We decided to look at charities so we spent some time linking trying to link data from the chariry commission with the gender pay gap data only to realise that it had already been [done]  ](https://www.civilsociety.co.uk/voices/david-kane-an-analysis-of-the-gender-pay-gap-in-charities.html) by someone called David Kane. (Writing this blog I have had the chance to check out David Kane's [blog](https://drkane.co.uk/blog/). It has some cool blog posts on charity data and I would recommend a visit). 

### My first encounter with python
Before we found the linked data I ran my first python code! I can't actually find the code I ran but I felt like it was a moment that needed to commemorate. I have continued learning python since and I kind of like it. Thanks to [Victoria](https://twitter.com/victoriambutt) for helping me get started with python.

## A map
Once we had the data I quickly focused on the address with postcodes. I am a sucker for maps so I decided to make one. 
...
To get the coordinates I first tried `geocode` from `ggmap`. The output from `geocode` has two columns so I got an error (I have included it below for one postcode). I am sure that there is an easy was around this but `geocode` is limited to 2000 calls per day and I had already used up 1800 calls so I had to find a different way. (I realised writing this that you can use `mutate_geocode`).  
```{r, error=TRUE}
...
The beauty of a hackathon is that you are surronded by people who can help you. I got some code from [Alessia](https://twitter.com/_alesssia) that worked beautifully. Notice the use of `unique`, some postcodes are in the data set mup=ltiple times and you only need to look them up once. 
...
We were now half way through the hack and I was ready to make a map. I used `leaflet` and created the different layers by using three `addCircles` commands and the addLayersCointrol.

```{r}
leaflet(gpg_map_comb,options= leafletOptions( minZoom=4, maxZoom=12) ) %>% 
...
## Summary
After creating the map I started to think about how to share it. I have made a lot of `laflet` maps in the past and in the spirit of trying new things I decided to make a shiny dashboard as I have never made on before and to be honest I struggle with shiny. I'll share my dashboard in my next post.   
