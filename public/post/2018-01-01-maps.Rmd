...
## Getting the data
A couple of months ago I discovered a package developed by Public Health England that makes it easier to extract the data behind their [fingertips website] (www.fingertips.phe.gov.uk). I wanted to try it and as I like maps I decided to download some data at CCG level and make an interactive choropleth map using the leaflet package. I use select_indicators to browse the indicators and pick one at random that seems to have CCG level data. Sometimes the page will not load so be aware that you might have to try several times. 
...
## Getting a shapefile
I need a shapefile to create a map so I just googled CCG shapefile and found one on data.gov.uk. I ended up having to simplify the spatial dataframe as it is very large making the map very large and slow.
...
shape2 <- gSimplify(shape_temp, tol=0.01, topologyPreserve=TRUE)
shape <- SpatialPolygonsDataFrame(shape2, data=shape_temp@data)
